...
if exists('simulation_data/seed_' + str(myseed) + '/currsums_sections_'+str(Nsynlocs)+'areagsynsmediumtau_fixeddt_type'+str(synloctype)+'_amp'+str(syngmax)+'_tstop'+str(tstop)+'_nseg'+str(nsegs)+'_dt'+str(dt)+'_seed'+str(myseed)+'_sim'+str(Nsims-1)+'x'+str(singleSimT)+'.mat'):
    sys.exit()
...
h("""
objref apicalina["""+str(apical_nsec*nsegs)+"""], apicalik["""+str(apical_nsec*nsegs)+"""], apicalica["""+str(apical_nsec*nsegs)+"""], apicalih["""+str(apical_nsec*nsegs)+"""], apicalil["""+str(apical_nsec*nsegs)+"""], apicalv["""+str(apical_nsec*nsegs)+"""], apicalicap["""+str(apical_nsec*nsegs)+"""], apicalimemb["""+str(apical_nsec*nsegs)+"""]
objref basalih["""+str(basal_nsec*nsegs)+"""], basalil["""+str(basal_nsec*nsegs)+"""], basalv["""+str(basal_nsec*nsegs)+"""], basalicap["""+str(basal_nsec*nsegs)+"""], basalimemb["""+str(basal_nsec*nsegs)+"""]
...
objref axonalil["""+str(axonal_nsec*nsegs)+"""], axonalv["""+str(axonal_nsec*nsegs)+"""], axonalicap["""+str(axonal_nsec*nsegs)+"""], axonalimemb["""+str(axonal_nsec*nsegs)+"""]
""")
...
# For each synapse, determine to which section it outputs the currents, and randomize the synapse activation times
...
for isim in range(Nsims):
    ...
    dict = {'ina': ina, 'ik': ik,
          'ica': ica, 'ih': ih,
          'il': ih, 'VtimesA': v, 'imemb': imemb, 'Vsoma': np.array(Vsoma),
          'icap': icap, 'times': np.array(times), 'A': A,
          'x': x, 'y': y, 'z': z,
          'ts_syn': np.array(ts_syn), 'sec_syn': np.array(allseg_syn)}
    scipy.io.savemat('simulation_data/seed_' + str(myseed) + '/currsums_parts_'+str(Nsynlocs)+'areagsynsmediumtau_fixeddt_type'+str(synloctype)+'_amp'+str(syngmax)+'_tstop'+str(tstop)+'_nseg'+str(nsegs)+'_dt'+str(dt)+'_seed'+str(myseed)+'_sim'+str(isim)+'x'+str(singleSimT)+'.mat', dict)
